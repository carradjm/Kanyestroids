(function () {
  ...
  MovingObject.prototype.draw = function(ctx) {
    if (this instanceof Asteroids.Asteroid) {
      ...
      var d = new Date();
      if(d.getSeconds() % 2 === 0) {
        ctx.drawImage(img, that.pos[0], that.pos[1], 60, 60);
      }
      else {
        ctx.drawImage(img2, that.pos[0], that.pos[1], 60, 60);
      }
    };
    ...
    if (this instanceof Asteroids.Ship) {
      ...
      var d = new Date();
      if(d.getSeconds() % 2 === 0) {
        ctx.drawImage(img3, that.pos[0], that.pos[1], 60, 60);
      }
      else {
        ctx.drawImage(img4, that.pos[0], that.pos[1], 60, 60);
      }
    }
